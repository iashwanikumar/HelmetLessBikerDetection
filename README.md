...
Motorcyclist Detection: The framework utilizes a Faster R-CNN model to pinpoint and crop bike riders from an overall frame or video stream.

Helmet Verification: The cropped image is passed to a custom-trained YOLOv3 network.

Violation Logging: If a "No Helmet" condition is triggered, the frame snapshot is automatically isolated and exported into an explicit output directory for evidence logging.
